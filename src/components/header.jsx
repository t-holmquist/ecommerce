    ...
    <div className='flex items-center gap-2 justify-between'>
        ...
        <button onClick={() => setCartIsVisible(true)} className='relative flex z-10 items-center justify-center cursor-pointer rounded-full bg-white w-12 h-12'>
          ...
          {cartProducts.length > 0 && (
            <div className='absolute flex items-center justify-center top-0 text-sm right-0 bg-red-500 text-white font-source-sans rounded-full w-5 h-5'>{cartProducts.length}</div>
          )}
        </button>
    </div>
  )
